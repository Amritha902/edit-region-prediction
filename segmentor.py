...
def create_image_encoder(image_size=(512, 512), trainable=False):
    ...
    # vit-keras names this layer 'Transformer/encoder_norm' on 0.1.x and
    # 'Transformer_encoder_norm' on 0.2.x, so accept either.
    for norm_layer_name in ("Transformer/encoder_norm", "Transformer_encoder_norm"):
        try:
            feature_map = base_model.get_layer(norm_layer_name).output
            break
        except ValueError:
            continue
    else:
        raise ValueError(
            "Could not find the ViT encoder norm layer. Layers present: "
            f"{[l.name for l in base_model.layers]}"
        )
...
class MaskDecoder(layers.Layer):
    def __init__(self, embedding_dim=EMBEDDING_DIM, num_heads=NUM_HEADS, num_blocks=NUM_DECODER_BLOCKS, image_embedding_patches=1024, **kwargs):
        ...
        self.upsampling_layers = keras.Sequential([
            ...
            layers.Activation("gelu"),
            # Sigmoid is required, not optional: both dice_loss and focal_loss below
            # assume probabilities in [0, 1]. Without it dice_loss's intersection term
            # goes negative and focal_loss's binary_crossentropy (from_logits=False)
            # clips every negative prediction to epsilon, killing the gradient there.
            layers.Conv2DTranspose(1, kernel_size=1, activation="sigmoid")
        ], name="mask_upsampler")
