...
try:                                   # CUDA-only; absent on Apple Silicon
...
from src.device import pick_device, pick_dtype, default_backend, describe, free_memory

class SegmentationModel:
    def __init__(self, model_path: str = 'models/instruct-seg-edit/best.pt', device: str = 'auto'):
        """
        ...
        # FastSAM(...).cuda() was hard-coded here. ultralytics takes the device
        # per-call instead, which keeps CUDA, MPS and CPU on one path.
        self.model = FastSAM(model_path)
        if self.device.startswith("cuda"):
            self.model = self.model.cuda(device=torch.device(self.device))
        print(f"Segmentation model loaded on {describe(self.device)}")
    ...
    def segment_image(self, image: Image.Image, text_prompt: list[str], return_result: bool = False):
        ...
        results = self.model.predict(image, texts=text_prompt, device=self.device)
...
class InpaintingModel:
    ...
    def __init__(
        ...
        self.device = pick_device(device)
        # flux needs Nunchaku int4 CUDA kernels - fall back to a backend that
        # actually loads on this machine.
        resolved_backend = (backend or os.environ.get("INPAINT_BACKEND")
                            or default_backend(self.device)).lower()
        if resolved_backend == "flux" and NunchakuFluxTransformer2dModel is None:
            fb = default_backend(self.device)
            print(f"flux backend needs Nunchaku (CUDA only); using '{fb}' instead.")
            resolved_backend = fb
        ...
        if self.device == "mps" and hasattr(self.pipe, "enable_attention_slicing"):
            # unified memory is shared with the rest of the system; slicing keeps
            # the peak well under the 19 GB MPS will hand out.
            self.pipe.enable_attention_slicing()
        if hasattr(self.pipe, "set_progress_bar_config"):
            self.pipe.set_progress_bar_config(disable=None)
        print(f"Inpainting backend '{self.backend}' on {describe(self.device)}")
    ...
    def _init_flux(self, model_name: str, torch_dtype: torch.dtype | None) -> None:
        ...
        dtype = torch_dtype or pick_dtype(self.device)
    ...
    def _init_sd2(self) -> None:
        ...
        self.pipe = StableDiffusionInpaintPipeline.from_pretrained(
            ...
            variant="fp16" if pick_dtype(self.device)==torch.float16 else None,
            use_safetensors=True,
