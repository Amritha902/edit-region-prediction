...
para(d,"One thing the sweep makes visible. Even undilated the predicted mask covers 22.4 percent of the frame against a true changed area of 9.6 percent, so at threshold 0.2 it over-covers by a factor of 2.3 before any dilation. Threshold and dilation are not independent, which raises the question of whether a joint search is needed or whether the operating point can be chosen from mask geometry alone.")

d.add_heading("8.2  A shortcut that did not work", level=2)
para(d,"A joint threshold and dilation search costs several hours of diffusion, so we first tested a proxy that costs none. If inpainting changed everything inside the mask and nothing outside, then the change-recall would equal the mask's own recall against ground truth, collateral would equal its false-positive rate, and net would equal their difference — Youden's J, computable from mask geometry alone.")
...
figure(d,"grid_heatmap.png","Figure 5. The operating-point grid. Net peaks at threshold 0.3 with 32 px of dilation and falls away in all four directions; collateral is monotonic in both parameters and carries no optimum.")
